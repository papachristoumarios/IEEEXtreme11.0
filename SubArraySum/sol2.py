...
def solve():
    ...
    for i in d1:
        p *= i
    old_arr = [int(i) for i in raw_input().split()]
    arr = [[[[[0] * d[-1] for _ in range(d[-2])] for _ in range(d[-3])] for _ in xrange(d[-4])] for _ in xrange(d[-5])]
    C = 0
    for i in range(d[0]):
        for j in range(d[1]):
            for k in range(d[2]):
                for l in range(d[3]):
                    for m in range(d[4]):
                        ...
                        C += 1
    sum_arr = [[[[[0] * d1[-1] for _ in range(d1[-2])] for _ in range(d1[-3])] for _ in xrange(d1[-4])] for _ in xrange(d1[-5])]
